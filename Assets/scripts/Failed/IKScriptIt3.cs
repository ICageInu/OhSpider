...
public class IKScriptIt3 : MonoBehaviour
{
    //amount of chains
    ...
    public TargetBehaviorFail GroundTarget;
    ...
    private Vector3 _lastPosGround;
    private Vector3 _offsetTargets;
    ...
    private void Init()
    {
        ...
        _lastPosGround = Target.position;
        _offsetTargets = Target.position - GroundTarget.TargetLocation;
        _offset = _bones[_bones.Length - 1].position - Target.position;
    }
    ...
    private void ResolveIK()
    {
        ...
        //calc
        //this checks the distance from target of leg tofirst node
        if ((_bones[_bones.Length - 1].position - _bones[0].position).sqrMagnitude >= _completeLength * _completeLength  && !_targetSet)
        {
            //here we insert the new position for the target
            //stretch
            //so if it is stretching, move the target to new position
            //Target.position = new Vector3(GroundTarget.TargetLocation.x, GroundTarget.TargetLocation.y + _offsetTargets.y, GroundTarget.TargetLocation.z);
            _lastPosGround = Target.position;
            //Debug.Log(_bones[_bones.Length - 1].position - _bones[0].position);
            _targetSet = true;

        }
        else
        {
            //move last node to lastposground or move it towards 
            _targetSet = false;
            Target.position = _lastPosGround;
            //if not stretching keep target at same position
            for (int it = 0; it < Iterations; it++)
            {
                //as long as it doesn't stretch, keep last bone on the ground
                //back
                for (int i = _positions.Length - 1; i > 0; i--)
                {
                    if (i == _positions.Length - 1)
                    {
                        _positions[i] = Target.position + _offset; //set to target
                ...
                //how close
                if ((_positions[_positions.Length - 1] - _bones[_bones.Length - 1].position).sqrMagnitude < DeltaDistance * DeltaDistance)
                    break;
            }
        }
